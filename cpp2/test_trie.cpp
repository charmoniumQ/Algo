...
void m() {
  Trie<char> trie (27, alpha_to_int, int_to_alpha);
  bool exit = false;
  char command;
  while (not exit) {
  cin >> command;
  ...
  case 'A': {
	...
	if (trie.insert(object.c_str(), object.size())) {
  ...
  case 'D': {
	string object; cin >> object;
	if (trie.remove(object.c_str(), object.size())) {
  ...
  case 'S': {
	string object; cin >> object;
	if (trie.is_present(object.c_str(), object.size())) {
  ...
  case 'M': {
	cout << "Membership is " << trie.size() << endl;
	break;
  }
  case 'C': {
	string lineStr;
	...
	do {
	  line >> object;
	  if (not trie.is_present(object.c_str(), object.size())) {
		cout << "Spelling mistake " << object << endl;
	  }
	} while (line.good());
	break;
  }
  case 'L': {
	for (auto it = trie.begin(); it.has_next(); it.next()) {
	  LinkedList<char> word = it.get();
	  for (auto ch = word.cbegin(); ch.has(); ++ch) {
